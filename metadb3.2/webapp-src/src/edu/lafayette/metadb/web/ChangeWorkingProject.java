...
/**
 * Servlet to switch the current working project in a session.
 * 
 * @author Long L. Ho
 ...
 *
 */
public class ChangeWorkingProject extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String project = request.getParameter(Global.SESSION_PROJECT);
		...
		try {
			if (session != null) {
				...
				out.print(project);
				ItemsDAO.cleanLock(UserManDAO.getLastProj(userName).split(";")[0].trim(), userName); //clean up locks from old working project.
			}
		}
		catch(Exception e) {
